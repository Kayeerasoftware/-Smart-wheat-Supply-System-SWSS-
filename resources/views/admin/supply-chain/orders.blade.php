...
<head>
    ...
    <title>Supply Chain Orders - Admin</title>
...
<body class="text-white overflow-x-hidden">
    ...
    <nav class="fixed-nav px-6 py-4 flex justify-between items-center">
        <div class="flex items-center space-x-4">
            ...
            <div>
                ...
                <p class="text-xs text-gray-400">Supply Chain Orders</p>
            </div>
        </div>
    ...
    <main class="main-content p-6">
        ...
        <div class="mb-8">
            <h1 class="text-3xl font-bold font-space gradient-text mb-2">Orders Management</h1>
            <p class="text-gray-400">View and manage all supply chain orders</p>
        </div>
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-700">
                    ...
                    <tbody class="divide-y divide-gray-800">
                        ...
                        <tr>
                            <td class="px-4 py-2 whitespace-nowrap font-semibold text-white">{{ $order->order_number ?? 'N/A' }}</td>
                            <td class="px-4 py-2 whitespace-nowrap text-gray-300">{{ $order->user->name ?? 'N/A' }}</td>
                            <td class="px-4 py-2 whitespace-nowrap">
                                <span class="inline-block px-3 py-1 rounded-full text-xs font-semibold {{ $order->status === 'completed' ? 'bg-green-500/20 text-green-300' : ($order->status === 'pending' ? 'bg-yellow-500/20 text-yellow-300' : 'bg-gray-500/20 text-gray-300') }}">
                                    {{ ucfirst($order->status ?? 'pending') }}
                                </span>
                            </td>
                            <td class="px-4 py-2 whitespace-nowrap text-gray-300">${{ number_format($order->total_amount ?? 0, 2) }}</td>
                            <td class="px-4 py-2 whitespace-nowrap text-gray-400">{{ $order->created_at ? $order->created_at->format('M d, Y') : 'N/A' }}</td>
                        </tr>
                        @empty
                        <tr>
