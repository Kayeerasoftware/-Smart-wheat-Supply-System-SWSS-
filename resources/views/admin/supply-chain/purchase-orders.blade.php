...
<head>
    ...
    <title>Supply Chain Purchase Orders - Admin</title>
...
<body class="text-white overflow-x-hidden">
    ...
    <nav class="fixed-nav px-6 py-4 flex justify-between items-center">
        <div class="flex items-center space-x-4">
            ...
            <div>
                ...
                <p class="text-xs text-gray-400">Supply Chain Purchase Orders</p>
            </div>
        </div>
    ...
    <main class="main-content p-6">
        ...
        <div class="mb-8">
            <h1 class="text-3xl font-bold font-space gradient-text mb-2">Purchase Orders Management</h1>
            <p class="text-gray-400">View and manage all supply chain purchase orders</p>
        </div>
        <div class="glass-card p-6">
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-700">
                    <thead>
                        <tr>
                            <th class="px-4 py-2 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">PO #</th>
                            <th class="px-4 py-2 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">Supplier</th>
                    ...
                    <tbody class="divide-y divide-gray-800">
                        ...
                        <tr>
                            <td class="px-4 py-2 whitespace-nowrap font-semibold text-white">{{ $po->po_number ?? 'N/A' }}</td>
                            <td class="px-4 py-2 whitespace-nowrap text-gray-300">{{ $po->supplier->name ?? 'N/A' }}</td>
                            <td class="px-4 py-2 whitespace-nowrap">
                                <span class="inline-block px-3 py-1 rounded-full text-xs font-semibold {{ $po->status === 'completed' ? 'bg-green-500/20 text-green-300' : ($po->status === 'pending' ? 'bg-yellow-500/20 text-yellow-300' : 'bg-gray-500/20 text-gray-300') }}">
                                    {{ ucfirst($po->status ?? 'pending') }}
                                </span>
                            </td>
                            <td class="px-4 py-2 whitespace-nowrap text-gray-300">${{ number_format($po->total_amount ?? 0, 2) }}</td>
                            <td class="px-4 py-2 whitespace-nowrap text-gray-400">{{ $po->created_at ? $po->created_at->format('M d, Y') : 'N/A' }}</td>
                        </tr>
                        @empty
                        <tr>
                            <td colspan="5" class="px-4 py-8 text-center text-gray-400">No purchase orders found.</td>
                        </tr>
                        @endforelse
                    </tbody>
                </table>
