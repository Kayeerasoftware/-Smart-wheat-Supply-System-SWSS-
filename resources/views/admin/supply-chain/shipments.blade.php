...
<head>
    ...
    <title>Supply Chain Shipments - Admin</title>
...
<body class="text-white overflow-x-hidden">
    ...
    <nav class="fixed-nav px-6 py-4 flex justify-between items-center">
        <div class="flex items-center space-x-4">
            ...
            <div>
                ...
                <p class="text-xs text-gray-400">Supply Chain Shipments</p>
            </div>
        </div>
    ...
    <main class="main-content p-6">
        ...
        <div class="mb-8">
            <h1 class="text-3xl font-bold font-space gradient-text mb-2">Shipments Management</h1>
            <p class="text-gray-400">View and manage all supply chain shipments</p>
        </div>
        <div class="glass-card p-6">
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-700">
                    <thead>
                        <tr>
                            <th class="px-4 py-2 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">Tracking #</th>
                            <th class="px-4 py-2 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">Warehouse</th>
                    ...
                    <tbody class="divide-y divide-gray-800">
                        ...
                        <tr>
                            <td class="px-4 py-2 whitespace-nowrap font-semibold text-white">{{ $shipment->tracking_number ?? 'N/A' }}</td>
                            <td class="px-4 py-2 whitespace-nowrap text-gray-300">{{ $shipment->warehouse->name ?? 'N/A' }}</td>
                            <td class="px-4 py-2 whitespace-nowrap">
                                <span class="inline-block px-3 py-1 rounded-full text-xs font-semibold {{ $shipment->status === 'delivered' ? 'bg-green-500/20 text-green-300' : ($shipment->status === 'in_transit' ? 'bg-yellow-500/20 text-yellow-300' : 'bg-gray-500/20 text-gray-300') }}">
                                    {{ ucfirst($shipment->status ?? 'pending') }}
                                </span>
                            </td>
                            <td class="px-4 py-2 whitespace-nowrap text-gray-400">{{ $shipment->created_at ? $shipment->created_at->format('M d, Y') : 'N/A' }}</td>
                        </tr>
                        @empty
                        <tr>
                            <td colspan="4" class="px-4 py-8 text-center text-gray-400">No shipments found.</td>
                        </tr>
                        @endforelse
                    </tbody>
                </table>
