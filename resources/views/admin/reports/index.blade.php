...
<body class="text-white overflow-x-hidden">
    ...
    <div class="flex">
        ...
        <aside class="sidebar w-64 p-6" id="sidebar">
            <nav class="space-y-2">
                ...
                <a href="{{ route('admin.reports.index') }}" class="sidebar-item active flex items-center space-x-3 px-4 py-3 rounded-lg text-white">
        ...
        <main class="flex-1 p-6 main-content">
            ...
            <div class="mb-8">
                <div class="flex items-center justify-between">
                    <div>
                        <h1 class="text-4xl font-bold text-white mb-2 font-space">Reports Dashboard</h1>
                    ...
                    <div class="flex items-center space-x-4">
                        ...
                        <div class="flex space-x-2">
                            <button onclick="exportReport('pdf')" class="btn-primary px-4 py-2 rounded-lg text-sm">
                                <i class="fas fa-file-pdf mr-2"></i>PDF
                            </button>
                            <button onclick="exportReport('excel')" class="btn-primary px-4 py-2 rounded-lg text-sm">
                                <i class="fas fa-file-excel mr-2"></i>Excel
                            </button>
                            <button onclick="exportReport('csv')" class="btn-primary px-4 py-2 rounded-lg text-sm">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                <div class="stat-card p-6 rounded-2xl text-center float-animation">
                    ...
                    <h3 class="text-3xl font-bold font-space mb-2">{{ $userStats['total_users'] }}</h3>
                    <p class="text-gray-400 mb-2">Total Users</p>
                    <div class="progress-bar">
                        <div class="progress-fill" style="width: {{ ($userStats['active_users'] / $userStats['total_users']) * 100 }}%"></div>
                    </div>
                </div>
                ...
                <div class="stat-card p-6 rounded-2xl text-center float-animation" style="animation-delay: 0.6s">
                    ...
                    <h3 class="text-3xl font-bold font-space mb-2">${{ number_format($orderStats['total_revenue'], 0) }}</h3>
                    <p class="text-gray-400 mb-2">Total Revenue</p>
                    <div class="progress-bar">
                        <div class="progress-fill" style="width: {{ ($orderStats['completed_orders'] / max($orderStats['total_orders'], 1)) * 100 }}%"></div>
                    </div>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
                ...
                <div class="glass-card p-6 rounded-2xl">
                    <div class="flex justify-between items-center mb-6">
                        <h2 class="text-2xl font-bold font-space">User Growth</h2>
                ...
                <div class="glass-card p-6 rounded-2xl">
                    <div class="flex justify-between items-center mb-6">
                        <h2 class="text-2xl font-bold font-space">Activity Trends</h2>
                        <div class="text-sm text-gray-400">{{ $dateRange }} days</div>
                    </div>
                    <div class="chart-container h-64 flex items-center justify-center">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                <a href="{{ route('admin.reports.user-report') }}" class="group">
                    <div class="glass-card p-6 rounded-2xl hover:bg-white/10 transition-all duration-300 transform hover:scale-105">
                        <div class="flex items-center mb-4">
                            ...
                            <h3 class="text-xl font-semibold text-white ml-4 font-space">User Report</h3>
                        </div>
                        <p class="text-gray-300">Detailed user statistics and analytics</p>
                    </div>
                </a>

                <a href="{{ route('admin.reports.activity-report') }}" class="group">
                    <div class="glass-card p-6 rounded-2xl hover:bg-white/10 transition-all duration-300 transform hover:scale-105">
                        <div class="flex items-center mb-4">
                            <div class="p-3 bg-green-100/20 rounded-full group-hover:bg-green-200/30 transition-colors">
                                <i class="fas fa-chart-line text-green-400 text-xl"></i>
                            </div>
                            <h3 class="text-xl font-semibold text-white ml-4 font-space">Activity Report</h3>
                        </div>
                        <p class="text-gray-300">System activity and user behavior analysis</p>
                    </div>
                </a>

                <a href="{{ route('admin.reports.vendor-report') }}" class="group">
                    <div class="glass-card p-6 rounded-2xl hover:bg-white/10 transition-all duration-300 transform hover:scale-105">
                        <div class="flex items-center mb-4">
                            ...
                            <h3 class="text-xl font-semibold text-white ml-4 font-space">Vendor Report</h3>
                        </div>
                        <p class="text-gray-300">Vendor performance and application analytics</p>
                    </div>
                </a>
            </div>
            ...
            <!-- Recent Activities -->
            <div class="glass-card p-6 rounded-2xl">
                <h2 class="text-2xl font-bold font-space mb-6">Recent System Activities</h2>
                <div class="space-y-4">
    ...
    <script>
        ...
        document.getElementById('dateRange').addEventListener('change', function() {
            window.location.href = '{{ route("admin.reports.index") }}?date_range=' + this.value;
        });

        // Export report function
        function exportReport(type) {
            const dateRange = document.getElementById('dateRange').value;
            const url = `{{ route('admin.reports.index') }}/export/${type}?date_range=${dateRange}`;
        ...
        new Chart(userGrowthCtx, {
            type: 'line',
            data: {
                labels: @json(array_column($chartData['user_growth'], 'date')),
                datasets: [{
                    label: 'New Users',
                    data: @json(array_column($chartData['user_growth'], 'count')),
                    borderColor: '#667eea',
        ...
        new Chart(activityTrendsCtx, {
            type: 'bar',
            data: {
                labels: @json(array_column($chartData['activity_trends'], 'date')),
                datasets: [{
                    label: 'Activities',
                    data: @json(array_column($chartData['activity_trends'], 'count')),
