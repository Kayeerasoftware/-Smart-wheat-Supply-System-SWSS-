    ...
    <div class="flex">
        ...
        <main class="flex-1 p-6 main-content">
            <div class="flex justify-between items-center mb-8">
                <h1 class="text-3xl md:text-4xl font-bold font-space gradient-text">Vendor Report</h1>
                <a href="{{ route('admin.reports.index') }}" class="px-4 py-2 rounded-lg text-sm font-semibold flex items-center space-x-2 bg-transparent border border-white text-white hover:bg-white/10 transition-all shadow-md">
                    <i class="fas fa-arrow-left"></i><span>Back to Reports</span>
                </a>
            </div>
            @if(isset($vendors) && $vendors->count())
                <div class="glass-card p-8 rounded-2xl shadow-lg">
                    <table class="min-w-full divide-y divide-gray-700">
                        <thead>
                            <tr>
                                <th class="px-4 py-2 text-left text-xs font-bold text-gray-200 uppercase">Name</th>
                                <th class="px-4 py-2 text-left text-xs font-bold text-gray-200 uppercase">Email</th>
                                <th class="px-4 py-2 text-left text-xs font-bold text-gray-200 uppercase">Status</th>
                                <th class="px-4 py-2 text-left text-xs font-bold text-gray-200 uppercase">Created</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-gray-800">
                            @foreach($vendors as $vendor)
                                <tr>
                                    <td class="px-4 py-2 text-gray-300">{{ $vendor->user->name ?? '-' }}</td>
                                    <td class="px-4 py-2 text-gray-300">{{ $vendor->user->email ?? '-' }}</td>
                                    <td class="px-4 py-2 text-gray-300">{{ ucfirst($vendor->status) }}</td>
                                    <td class="px-4 py-2 text-gray-300">{{ $vendor->created_at->format('Y-m-d') }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
            ...
            @else
                <div class="glass-card p-8 rounded-2xl bg-yellow-500/10 text-yellow-300 text-center">
                    No vendors found.
                </div>
            @endif
        </main>
