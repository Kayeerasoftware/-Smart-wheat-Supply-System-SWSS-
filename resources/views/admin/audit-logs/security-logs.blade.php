...
<head>
    ...
    <title>SWSS Security Logs</title>
...
<body class="text-white overflow-x-hidden">
    ...
    <div class="flex">
        ...
        <main class="flex-1 p-6 main-content">
            <div class="mb-8">
                <h1 class="text-4xl font-bold text-white mb-2 font-space">Security Logs</h1>
                <p class="text-gray-400">Login attempts, security events, and access control</p>
            </div>
            <div class="mb-8 flex flex-col md:flex-row md:items-end md:space-x-4 space-y-4 md:space-y-0 justify-between">
                <div class="flex-1"></div>
                <a href="{{ route('admin.audit-logs.index') }}" class="px-4 py-2 rounded-lg text-sm font-semibold flex items-center space-x-2 bg-transparent border border-white text-white hover:bg-white/10 transition-all shadow-md mb-2 md:mb-0 ml-auto">
            ...
            <div class="mb-6 flex flex-col md:flex-row md:items-end md:space-x-4 space-y-4 md:space-y-0">
                <form id="filterForm" method="GET" class="flex flex-wrap items-end space-x-2">
                    <div>
                        ...
                        <select name="user_id" class="bg-gray-800 text-gray-100 rounded px-3 py-2 border border-gray-700">
                    ...
                    <div>
                        ...
                        <select name="type" class="bg-gray-800 text-gray-100 rounded px-3 py-2 border border-gray-700">
                            ...
                            <option value="data_access" {{ request('type') == 'data_access' ? 'selected' : '' }}>Data Access</option>
                            <option value="system_change" {{ request('type') == 'system_change' ? 'selected' : '' }}>System Change</option>
                        </select>
                    </div>
                    <div>
                        ...
                        <table class="min-w-full divide-y divide-gray-700">
                            <thead>
                                <tr>
                                    ...
                                    <th class="px-4 py-2 text-left text-xs font-bold text-gray-200 uppercase">User</th>
                            ...
                            <tbody class="divide-y divide-gray-800">
                                @foreach($logs as $log)
                                    <tr>
                                        ...
                                        <td class="px-4 py-2 text-gray-300">{{ $log->user->username ?? 'N/A' }}</td>
                                        <td class="px-4 py-2 text-gray-300">{{ $log->type }}</td>
