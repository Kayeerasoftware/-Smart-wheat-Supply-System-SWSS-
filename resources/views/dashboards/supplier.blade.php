...
<head>
    ...
    <style>
        ...
        .tab-content {
            /* Alpine.js will handle visibility */
        }
...
<body class="text-white overflow-x-hidden">
    ...
    <!-- Main Content -->
        ...
        <div x-show="activeTab === 'overview'" class="tab-content">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                <div class="metric-card">
                    ...
                    <div class="text-3xl font-bold text-green-400 mb-2">{{ $orderFulfillmentRate ?? 0 }}%</div>
                    <div class="progress-bar">
                        <div class="progress-fill" style="width: {{ $orderFulfillmentRate ?? 0 }}%"></div>
                    </div>
                ...
                <div class="metric-card">
                    ...
                    <div class="text-3xl font-bold text-yellow-400 mb-2">{{ number_format($customerSatisfaction, 1) }}/5</div>
                    <div class="flex space-x-1">
                        @for($i = 1; $i <= 5; $i++)
                            <i class="fas fa-star {{ $i <= round($customerSatisfaction) ? 'text-yellow-400' : 'text-gray-600' }}"></i>
                        @endfor
                    </div>
                    <p class="text-sm text-gray-400 mt-2">Based on {{ $customerSatisfactionCount }} reviews</p>
                </div>

                <div class="metric-card">
                    <div class="flex items-center justify-between mb-4">
                        <h4 class="text-lg font-semibold text-white">Inventory Turnover</h4>
                        <i class="fas fa-sync-alt text-blue-400 text-xl"></i>
                    </div>
                    <div class="text-3xl font-bold text-blue-400 mb-2">{{ $inventoryTurnover ?? 0 }}x</div>
                    <p class="text-sm text-gray-400 mt-2">Annual turnover rate</p>
                </div>
            </div>
        ...
        <div x-show="activeTab === 'analytics'" class="tab-content">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
                ...
                <div class="glass-card p-6 mb-8" style="min-height: 400px;">
                    <h3 class="text-2xl font-semibold text-white mb-4">Inventory Trend (Last 30 Days)</h3>
                    <div class="chart-container p-4" style="height: 350px; width: 100%;">
                        <canvas id="inventoryTrendChart" width="1200" height="350"></canvas>
                    </div>
                </div>

                <!-- Revenue Chart -->
                <div class="glass-card p-6 mb-8" style="min-height: 400px;">
                    <h3 class="text-2xl font-semibold text-white mb-4">Revenue Analytics</h3>
                    <div class="chart-container p-4" style="height: 350px; width: 100%;">
                        <canvas id="revenueChart" width="1200" height="350"></canvas>
                    </div>
                </div>
            </div>
            ...
            <div class="glass-card p-6 mb-6">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    <!-- Forecast Chart -->
                    <div class="glass-card p-6 mb-8" style="min-height: 400px;">
                        <h4 class="text-2xl font-semibold text-white mb-4">6-Month Demand Forecast</h4>
                        <div class="chart-container p-4" style="height: 350px; width: 100%;">
                            <canvas id="demandForecastChart" width="1200" height="350"></canvas>
                        </div>
                        @if(!isset($demandForecast) || !$demandForecast)
                            <div class="text-center text-gray-400 mt-4">No forecast data available. Please check back later.</div>
                        @endif
                    </div>
                    <!-- Forecast Summary -->
                    <div>
                        <h4 class="text-lg font-semibold text-white mb-4">Forecast Summary</h4>
                        @if(isset($demandForecast) && $demandForecast)
                        <div class="space-y-4">
                            <div class="bg-blue-500/10 p-4 rounded-lg">
                                <div class="text-2xl font-bold text-blue-400">{{ number_format($demandForecast['summary']['forecast_avg'] ?? 0) }}</div>
                                <div class="text-sm text-gray-300">Average Monthly Demand</div>
                            </div>
                            <div class="bg-green-500/10 p-4 rounded-lg">
                                <div class="text-2xl font-bold text-green-400">{{ number_format($demandForecast['summary']['forecast_total'] ?? 0) }}</div>
                                <div class="text-sm text-gray-300">Total 6-Month Forecast</div>
                            </div>
                            <div class="bg-purple-500/10 p-4 rounded-lg">
                                <div class="text-2xl font-bold text-purple-400">{{ number_format($demandForecast['summary']['growth_rate'] ?? 0, 1) }}%</div>
                                <div class="text-sm text-gray-300">Expected Growth Rate</div>
                            </div>
                        </div>
                        @else
                        <div class="bg-blue-500/10 p-4 rounded-lg text-center text-gray-400">No forecast summary available.</div>
                        @endif
                    </div>
                </div>
                <!-- AI Recommendations -->
                @if(isset($forecastRecommendations) && count($forecastRecommendations) > 0)
                <div class="mt-6">
                    <h4 class="text-lg font-semibold text-white mb-4">AI Recommendations</h4>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        @foreach($forecastRecommendations as $recommendation)
            ...
            <div class="glass-card p-6 mb-6">
                <div class="flex items-center justify-between mb-6">
                    <h3 class="text-xl font-semibold text-white">Supplier Product Purchase Insights</h3>
                </div>
                @if(isset($productPurchases) && count($productPurchases) > 0)
                    @php
                        $isRandom = isset($productPurchases[0]['farmer_name']);
                    @endphp
                        ...
                        <table class="w-full text-sm">
                            <thead>
                                <tr class="border-b border-gray-700">
                                    ...
                                    @if($isRandom)
                                        <th class="text-left py-3 px-4 text-gray-300 font-medium">Farmer</th>
                                        <th class="text-left py-3 px-4 text-gray-300 font-medium">Quantity (kg)</th>
                                        <th class="text-left py-3 px-4 text-gray-300 font-medium">Price per Kg</th>
                                        <th class="text-left py-3 px-4 text-gray-300 font-medium">Quality Grade</th>
                                        <th class="text-left py-3 px-4 text-gray-300 font-medium">Action</th>
                                    @else
                                        <th class="text-left py-3 px-4 text-gray-300 font-medium">Total Quantity</th>
                @endif
                                </tr>
                            </thead>
                            <tbody>
                                @foreach($productPurchases as $purchase)
                                    <tr class="border-b border-gray-800 hover:bg-gray-800/40">
                                        <td class="py-2 px-4 text-white">{{ $purchase['product_name'] ?? $purchase['product'] ?? '' }}</td>
                                        @if($isRandom)
                                            <td class="py-2 px-4 text-white">{{ $purchase['farmer_name'] ?? '' }}</td>
                                            <td class="py-2 px-4 text-white">{{ $purchase['quantity'] ?? '' }}</td>
                                            <td class="py-2 px-4 text-white">{{ $purchase['price_per_kg'] ?? '' }}</td>
                                            <td class="py-2 px-4 text-white">{{ $purchase['quality_grade'] ?? '' }}</td>
                                            <td class="py-2 px-4">
                                                <a href="{{ url('/orders/create?product_id=' . ($purchase['product_id'] ?? '') . '&farmer_id=' . ($purchase['farmer_id'] ?? '')) }}"
                                                   class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-1 rounded shadow transition">
                                                    Order
                                                </a>
                                            </td>
                @else
                                            <td class="py-2 px-4 text-white">{{ $purchase['total_quantity'] ?? '' }}</td>
                                        @endif
                                    </tr>
                                @endforeach
                            </tbody>
                ...
                @else
                    <div class="text-gray-300">No product purchase insights or available wheat products found.</div>
                @endif
            </div>
        </div>
    ...
    <!-- Modal Order Form (iframe version) -->
    <div x-data="{ orderModalOpen: false, orderModalUrl: '' }" x-cloak>
        <template x-if="orderModalOpen">
            <div class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-60">
                <div class="bg-white rounded-2xl shadow-xl w-full max-w-3xl h-[90vh] p-0 relative flex flex-col">
                    <button @click="orderModalOpen = false" class="absolute top-4 right-4 text-gray-500 hover:text-gray-700 z-10">
                        <i class="fas fa-times text-2xl"></i>
                    </button>
                    <iframe :src="orderModalUrl" class="w-full h-full rounded-b-2xl border-0" style="min-height: 80vh;"></iframe>
                </div>
            </div>
        </template>
    </div>
    <script>
        if (typeof window.Alpine === 'undefined') {
            console.warn('Alpine.js is not loaded! The order modal will not work.');
        }
        // Initialize inventory trend chart
        const ctx = document.getElementById('inventoryTrendChart').getContext('2d');
        const inventoryTrendData = @json($inventoryTrends ?? []);
            ...
            new Chart(ctx, {
                ...
                data: {
                    ...
                    datasets: [{
                        ...
                        data: inventoryTrendData.map(item => item.total),
                        borderColor: '#4facfe',
        ...
        @if(isset($demandForecast) && $demandForecast)
        const demandForecastCtx = document.getElementById('demandForecastChart');
        if (demandForecastCtx) {
            new Chart(demandForecastCtx.getContext('2d'), {
                type: 'line',
                data: {
                    labels: @json($demandForecast['dates'] ?? []),
                    datasets: [{
                        label: 'Historical Demand',
                        data: @json($demandForecast['historical_data']['values'] ?? []),
                        ...
                        data: @json($demandForecast['values'] ?? []),
                        ...
                        data: @json($demandForecast['upper_ci'] ?? []),
        ...
        @if(isset($customerSegments) && $customerSegments)
        const customerSegmentsCtx = document.getElementById('customerSegmentsChart');
        if (customerSegmentsCtx) {
            const segmentData = @json($customerSegments['segment_summary'] ?? []);
            const labels = Object.keys(segmentData);
            const data = labels.map(label => segmentData[label]['count'] ?? 0);
            const colors = [
                'rgba(59, 130, 246, 0.8)',
                'rgba(34, 197, 94, 0.8)',
                'rgba(168, 85, 247, 0.8)',
